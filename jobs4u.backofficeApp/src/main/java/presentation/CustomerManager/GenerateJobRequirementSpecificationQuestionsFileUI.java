...
public class GenerateJobRequirementSpecificationQuestionsFileUI {
    ...
    protected boolean doShow(AuthzUI authzUI){
        ...
        if (choice >= 0 && choice < i) {
            try {
                JobOpening jobOpening = generateCandidateFieldsFileController.findJobOpeningById(String.valueOf(jobOpeningList.get(choice).getId()));
                String pluginPath = jobOpening.getJobSpecifications();
                if(pluginPath == null){
                    System.out.println("No Job Specification Model associated with this Job Opening");
                    return false;
                }
                else{
